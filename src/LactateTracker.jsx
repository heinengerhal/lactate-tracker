...
// STORAGE & CSV
// ═══════════════════════════════════════════════════════════════════
const STORAGE_KEY = "la-tracker-v2";
...
function RacePrediction({ model }) {
  ...
  const FM_DIST = 42.195;

  const hmTimeSec = hmPaceSec * HM_DIST;
  const fmTimeSec = fmPaceSec * FM_DIST;

  const fmViaRiegelSec = hmTimeSec * Math.pow(FM_DIST / HM_DIST, RIEGEL);
  const fmFinalSec = (fmTimeSec + fmViaRiegelSec) / 2;
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <div style={{ fontSize: 10, letterSpacing: 3, color: t.muted, fontFamily: "'JetBrains Mono',monospace" }}>
          RACE PREDICTIONS
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {races.map(r => (
          <div key={r.label} style={{ background: t.surface, borderRadius: 14,
            ...
            <div style={{ flex: 1, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: 17, fontWeight: 900, letterSpacing: 1,
                    fontFamily: "'Barlow Condensed',sans-serif", color: t.text }}>
                    {r.label}
                  </div>
                  <div style={{ fontSize: 10, color: t.muted, fontFamily: "'JetBrains Mono',monospace", marginTop: 2 }}>
                    {r.dist} · based on {r.lacRef}
                  </div>
                </div>
                {r.sub && (
                  <div style={{ background: r.color + "22", border: `1px solid ${r.color}55`,
                    borderRadius: 20, padding: "3px 10px", fontSize: 11,
                    fontFamily: "'JetBrains Mono',monospace", color: r.color, fontWeight: 700 }}>
                    {r.sub}
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                <span style={{ fontSize: 42, fontWeight: 900, fontFamily: "'JetBrains Mono',monospace",
                  color: r.color, lineHeight: 1 }}>
                  {formatRaceTime(r.time)}
                </span>
                <div>
                  <div style={{ fontSize: 13, fontFamily: "'JetBrains Mono',monospace", color: t.muted }}>
                    {secToPace(r.pace)} /km
                  </div>
                </div>
              </div>
      ...
      <div style={{ marginTop: 10, fontSize: 10, color: t.muted,
        ...
        Assumes even pacing and no significant fatigue deviation. Log more data points to improve accuracy.
      </div>
    </div>
  );
}
        ...
        <div style={{ marginTop: 22 }} className="fade-in">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <SectionLabel style={{ margin: 0 }}>
              THIS SESSION — {staged.length} {staged.length === 1 ? "ENTRY" : "ENTRIES"}
            </SectionLabel>
          ...
          <div style={{ background: t.surface, borderRadius: 12, overflow: "hidden", border: `1px solid ${t.border}` }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 60px 36px 56px 28px",
              ...
              {["PACE","BPM","LA","TYPE","RPE","FAT.",""].map(h => (
                <div key={h} style={{ fontSize: 8, letterSpacing: 1.5, color: t.muted,
                  fontFamily: "'JetBrains Mono',monospace", textAlign: "center" }}>{h}</div>
              ))}
            </div>
            {staged.map((e, i) => {
              const z = zone(e.lactate);
              return (
                <div key={e.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 60px 36px 56px 28px",
                  ...
                  <span style={{ fontSize: 11, fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700,
                    color: INTERVAL_COLORS[e.intervalType], textAlign: "center" }}>{e.intervalType}</span>
  ...
  return (
    <div className="fade-in">
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        ...
        <button onClick={() => exportCSV(sessions)}
          style={{ padding: "8px 12px", background: t.surface, border: `1px solid ${t.border}`,
            ...
            letterSpacing: 1, cursor: "pointer", display: "flex", flexDirection: "column",
      ...
      {sessions.map(session => {
        ...
        return (
          <div key={session.id} style={{ background: t.surface, borderRadius: 14, marginBottom: 10,
            ...
            <div style={{ padding: "13px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontFamily: "'JetBrains Mono',monospace", color: t.text, fontWeight: 600, marginBottom: 5 }}>
                  {fmt(session.date)}
                </div>
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
                  {[...new Set(session.entries.map(e => e.intervalType))].map(it => (
                    <span key={it} style={{ fontSize: 10, fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700,
                      letterSpacing: 1, padding: "2px 7px", borderRadius: 5,
                      background: INTERVAL_COLORS[it] + "22", color: INTERVAL_COLORS[it],
                      border: `1px solid ${INTERVAL_COLORS[it]}44` }}>{it}</span>
                  ))}
              ...
              <div style={{ textAlign: "right", marginRight: 6 }}>
                <div style={{ fontSize: 24, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: z.color, lineHeight: 1 }}>
                  {avgLac.toFixed(1)}
                </div>
            ...
            {open && (
              <div className="fade-in" style={{ borderTop: `1px solid ${t.border}` }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 60px 36px 56px 80px",
                      ...
                      <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                        <button onClick={() => setEditTarget({ sessionId: session.id, entryId: e.id })}
                          style={{ background: "transparent", border: `1px solid ${t.border}`, color: t.muted,
                            borderRadius: 6, padding: "3px 7px", fontSize: 10, fontFamily: "'JetBrains Mono',monospace", cursor: "pointer" }}>
                          Edit
                        </button>
                ...
                <div style={{ padding: "10px 14px", display: "flex", justifyContent: "flex-end",
                  alignItems: "center", gap: 8, background: t.bg, borderTop: `1px solid ${t.border}` }}>
                  {confirmDel ? (
                    <>
                      <span style={{ fontSize: 11, color: t.muted, fontFamily: "'JetBrains Mono',monospace", marginRight: 4 }}>Delete entire session?</span>
                      <button onClick={() => setDelConfirm(null)}
                        style={{ background: "transparent", border: `1px solid ${t.border}`, color: t.muted,
                          borderRadius: 8, padding: "5px 12px", fontSize: 11, fontFamily: "'JetBrains Mono',monospace", cursor: "pointer" }}>
                        Cancel
                      </button>
                      <button onClick={() => deleteSession(session.id)}
                        style={{ background: "#FF2D55", border: "none", color: "#fff", borderRadius: 8,
                    ...
                    <button onClick={() => setDelConfirm(session.id)}
                      style={{ background: "transparent", border: "1px solid #FF2D5533", color: "#FF2D55",
                        borderRadius: 8, padding: "5px 14px", fontSize: 12, fontFamily: "'Barlow Condensed',sans-serif",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            ...
            <button onClick={() => setIsDark(d => !d)}
              style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 20,
                padding: "6px 12px", cursor: "pointer", fontSize: 15, color: t.text, marginTop: 2 }}>
              {isDark ? "☀️" : "🌙"}
            </button>
          </div>
